...
* **Model:**
    * **Logistic Regression:** Chosen for its simplicity and efficiency as a baseline classifier. It is configured with balanced class weights (`class_weight='balanced'`) to handle data imbalance.
* **Features:**
    * Raw EEG signal data from a single channel at a time. No transformations, filtering, or feature extraction methods were applied.
* **Procedure:**
    1.  **Data Splitting:** For each subject, data from series 1–6 were used for training, and series 7–8 were reserved for validation.
    2.  **Training:** A separate model was trained for each of the 32 EEG channels across all 12 subjects. The training pipeline first standardized the data using `StandardScaler` before feeding it into the `LogisticRegression` classifier.
...
The experiment was executed for all 12 subjects across all 32 channels. The average AUC scores and the detailed channel performance rankings for each subject are summarized in the table below:

### 📊 Full Channel Ranking per Subject (Event: HandStart)
...
> **Analysis Note:** > The results indicate that even when using raw, unfiltered signals, most subjects exhibit an average AUC score above 0.5. This suggests that the unprocessed EEG data contains underlying predictive information. However, the overall performance remains relatively low and highly inconsistent across subjects, which underlines the limitations of bypassing formal signal processing.

---

## 4. Conclusion & Next Steps

This experiment successfully established a clear baseline performance for decoding the "HandStart" event. 

* **Key Conclusion:** While raw EEG signals hold baseline predictive power, they lack the robust signal-to-noise ratio necessary to build a highly accurate or generalizable classification model.
* **Next Steps:** To improve upon these baseline benchmarks, the subsequent phase (**feature_filterbank_v1**) will implement a dedicated filter bank architecture. This will break down the raw signals into distinct frequency-based features to evaluate whether spectral decomposition can systematically drive higher AUC scores.
